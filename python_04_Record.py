...
def oper_num():
    """记录程序运行的次数"""
    f = open(r"D:\\python_01_project\number_start.txt",'r+')#打开文件
    R = int(f.read())
    f.close()
    f2 = open(r"D:\\python_01_project\number_start.txt",'w+',)
    R = R+1
    f2.write(str(R))
    f2.close
    return R
def record_news():
    file_news = open("D:\\python_01_project\OneDayOneWord.txt",'a+')
    contents = get_news()
    file_news.write('\n')
    oper_number = str(oper_num())
    file_news.write("第"+oper_number+"次")
    file_news.write('\n')
    file_news.close()
    file_news = open("D:\\python_01_project\OneDayOneWord.txt",'a+',encoding = 'utf-8')
